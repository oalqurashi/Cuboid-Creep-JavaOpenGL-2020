...
public class Worm2 implements Serializable{
    public static float x=-9f,y=-11f;
    ...
    public static Path getPath(ArrayList<Path> paths){
        ...
        for (Path path : paths) {
            if (Worm2.x >= path.getCorner1().getMinX() && Worm2.x <= path.getCorner2().getMaxX() && Worm2.y >= path.getCorner1().getMinY() && Worm2.y <= path.getCorner2().getMaxY()){
                currentPath = path ;
            }
        }
        return currentPath ;
    ...
    public static boolean isInSafeRoom(){
        ...
        if (Worm2.x >= safeRoom.getSafeRoom().getMinX()-2 && Worm2.x <= safeRoom.getSafeRoom().getMaxX()+2 && Worm2.y >= safeRoom.getSafeRoom().getMinY() && Worm2.y <= safeRoom.getSafeRoom().getMaxY()){
                return true;
            }
        else return false;
